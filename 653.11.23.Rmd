...
## (2) LM fitting
- We test the significance of covariates and determine to delete UR (b.c.unbalanced data issue)
```{r}
lm1<-lm(Y~Year+logmu+logratio+CI+UR,data=workingdata)
summary(lm1)
AIC(lm1)
```

# 1. Residual, Interaction checking(?)
...
## (2) Residual Plot
```{r}
res<-(workingdata[,15]-predict(lm1,workingdata))
...
## (2) Interaction
```{r}
int1<-lm(Y~Year+logmu+logratio+UR+CI+UR*logmu+Year*logmu+CI*logmu,data=workingdata)
summary(int1)
AIC(int1)
```

## Final decision: delete UR and UR interaction term
```{r}
int2<-lm(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,data=workingdata)
summary(int2)
```


# 2. Check random effect necessity
## (1) random slope necessary
```{r,warning=FALSE}
library(nlme)
full<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,
          random=~Year|HHid,correlation=corSymm(form=~1),data=workingdata,control=Ctr)
reduce<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,random=~1|HHid,
            correlation=corSymm(form=~1),data=workingdata,control=Ctr)
summary(full)
summary(reduce)
```
...
var1<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,
          random=~Year|HHid,correlation=corSymm(form=~1),data=workingdata,control=Ctr)
summary(var1)
```
...
## (2) Compound symmetry
```{r}
var2<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,
...
## (3) AR1
```{r}
var3<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,
...
## (4) Exp
```{r}
var4<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,
...
## (5) CAR
```{r}
var5<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,
...
# 4. Variance of Y
```{r}
D=matrix(c(0.019724,-0.010605,-0.010605,0.005725),nrow=2)
Z=matrix(c(1,0,1,2,1,5,1,7,1,11),nrow=2)
sigmasq=0.5976228^2
R<-matrix(c(1,0.155,0.174,0.274,0.336,0.155,1,0.028,0.016,0.017,0.174,0.028,1,-0.195,-0.355,0.274,0.016,-0.195,1,-0.586,0.366,0.017,-0.355,-0.586,1),nrow=5)
R<-sigmasq*R
vary<-t(Z)%*%D%*%Z+R
```
...
```{r}
hatY<-predict(var1d,datad)
res<-datad$Y-hatY
resid<-cbind(res,datad$time,datad$Year)
for (i in 1:5161){
...
plot(var1d)
plot(var1,pch=5,cex=0.8,main="Standardized Residuals vs Fitted",xlab="Fitted value",ylab="Standardized")
```

# 6. Cook's Distance-- Deleting influntial points
```{r}
## 452404017, 521108004, 422102005
id1<-which(workingdata$HHid=="422102005")
id2<-which(workingdata$HHid=="452404017")
id3<-which(workingdata$HHid=="521108004")
datad<-workingdata[-c(id1,id2,id3),]

var1d<-lme(Y~Year+logmu+logratio+CI+Year*logmu+CI*logmu,random=~Year|HHid,
           correlation=corSymm(form=~1),data=datad,control=Ctr)
summary(var1d)
```
...
```{r}
coeff<-var1d$coefficients$fixed
intercept1<-coeff[1]
slope1<-coeff[3]
intercept2<-coeff[1]+coeff[5]
slope2<-coeff[3]+coeff[7]
...
ggplot(data,aes(x=x,y=y,color=Group))+geom_line()+theme_bw()+labs(title="Total Value vs mu Land in log scale",x="logmu",y="Log total value from crops")+
